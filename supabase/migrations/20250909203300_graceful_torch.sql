...
CREATE TABLE IF NOT EXISTS reviews (
  ...
  collaboration_type text NOT NULL, -- 'as_influencer', 'as_advertiser'
...
CREATE TABLE IF NOT EXISTS payment_confirmations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  deal_id uuid NOT NULL REFERENCES deals(id) ON DELETE CASCADE,
  payment_stage text NOT NULL, -- 'prepay', 'postpay'
  confirmed_by uuid NOT NULL REFERENCES user_profiles(user_id) ON DELETE CASCADE,
  confirmation_type text NOT NULL, -- 'payment_sent', 'payment_received'
...
CREATE INDEX idx_deals_payer_id ON deals(payer_id);
CREATE INDEX idx_deals_payee_id ON deals(payee_id);
...
CREATE INDEX idx_payment_confirmations_deal_id ON payment_confirmations(deal_id);
CREATE INDEX idx_payment_confirmations_confirmed_by ON payment_confirmations(confirmed_by);
