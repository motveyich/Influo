    ...
    >
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {allOffers.map((offer) => (
              <OfferCard
                key={offer.offerId}
                offer={offer}
                onAction={activeTab === 'received' ? 
                  ((offer as any).type === 'application' ? 
                    (id, action) => handleApplicationAction(id, action) :
                    (id, action) => handleOfferAction(id, action)
                  ) : undefined
                }
                onCreatePayment={() => handleCreatePayment(offer)}
                onManageDeal={() => handleManageDeal(offer.offerId)}
                onWithdraw={activeTab === 'sent' ? () => handleWithdraw(offer.offerId) : undefined}
                onLeaveReview={() => handleLeaveReview(offer)}
        ...
        {!isLoading && allOffers.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              {activeTab === 'received' 
                ? 'Предложения и заявки от других пользователей появятся здесь'
                : 'Ваши отправленные предложения и заявки появятся здесь'
              }
            </p>
          </div>
        )}
        ...
        <PaymentModal
          ...
          offerId={selectedOfferForPayment?.offerId}
          payerId={selectedOfferForPayment?.advertiserId || ''}
          payeeId={selectedOfferForPayment?.influencerId || ''}
          totalAmount={selectedOfferForPayment?.details.rate || 0}
          currency={selectedOfferForPayment?.details.currency}
          currentUserId={currentUserId}
          onDealCreated={() => {
          ...
          onCreatePayment={() => {
            ...
            if (selectedDeal) {
              setSelectedOfferForPayment({
                offerId: selectedDeal.id,
                advertiserId: selectedDeal.payerId,
                influencerId: selectedDeal.payeeId,
                details: { rate: selectedDeal.totalAmount, currency: selectedDeal.currency }
              } as any);
        ...
        {paymentRequestOffer && (
          <CreatePaymentRequestModal
            ...
            currency={paymentRequestOffer.details.currency || 'USD'}
            onRequestCreated={() => {
