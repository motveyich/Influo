...
export function InfluencerCardDisplay({
  ...
  const handleIntegrationSubmit = async (integrationDetails: any) => {
    setIsLoading(true);
    try {
      const message = `Заинтересован в сотрудничестве с вашей карточкой на платформе ${card.platform}.

Ниша: ${integrationDetails.niche}
Описание продукта: ${integrationDetails.productDescription}
Параметры интеграции: ${integrationDetails.integrationParameters}

Предлагаемая ставка: ${card.serviceDetails.pricing.post || 1000} руб.
Срок: 2 недели
Результаты: Пост в ${card.platform}`;
    ...
    <div className={`bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 p-6 border ${
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-600">{t('influencerCards.subscribers')}</p>
        </div>
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-gray-600">{t('influencerCards.engagement')}</p>
        </div>
      </div>
          ...
          <div className="mt-4 space-y-2">
            <div className="flex space-x-2">
              ...
              >
               ...
               <span>{card.isActive ? t('influencerCards.active') : t('influencerCards.inactive')}</span>
              </button>
            </div>
            ...
            <div className="flex space-x-2">
              ...
              <button
                ...
                title={t('influencerCards.delete')}
              >
                <Trash2 className="w-4 h-4" />
                <span>{t('influencerCards.delete')}</span>
              </button>
            </div>
          </div>
        )}
