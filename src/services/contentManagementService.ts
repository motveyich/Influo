...
export class ContentManagementService {
  // News Management
  async createNews(newsData: Partial<PlatformNews>, createdBy: string): Promise<PlatformNews> {
    try {
      // Check permissions
      ...
      const newNews = {
        ...
        category: newsData.category || 'industry',
        published_at: newsData.publishedAt || new Date().toISOString(),
        is_published: newsData.isPublished ?? true,
      ...
      // Log the action
      await adminService.logAction(createdBy, 'news_created', 'platform_news', data.id, {
        title: newsData.title,
        category: newsData.category
      });

      return this.transformNewsFromDatabase(data);
    } catch (error) {
  ...
  async updateNews(newsId: string, updates: Partial<PlatformNews>, updatedBy: string): Promise<PlatformNews> {
    try {
      // Check permissions
      ...
      // Log the action
      await adminService.logAction(updatedBy, 'news_updated', 'platform_news', newsId, {
  ...
  async deleteNews(newsId: string, deletedBy: string): Promise<void> {
    try {
      // Check permissions
      ...
      // Log the action
      await adminService.logAction(deletedBy, 'news_deleted', 'platform_news', newsId);
    } catch (error) {
  ...
  async getAllNews(filters?: {
    ...
    try {
      ...
      return data.map(news => this.transformNewsFromDatabase(news));
    } catch (error) {
  ...
  async getPublishedNews(): Promise<PlatformNews[]> {
    try {
      const { data, error } = await supabase
        .from(TABLES.PLATFORM_NEWS)
        .select('*')
      ...
      console.error('Failed to get published news:', error);
      return [];
    }
  }
  ...
  // Transform functions
  private transformNewsFromDatabase(dbData: any): PlatformNews {
    return {
      id: dbData.id,
      title: dbData.title,
