...
export class AnalyticsService {
  ...
  private initializeGA() {
    try {
      ...
      script.onerror = () => {
        console.warn(
          "Failed to load Google Analytics. Events will be stored locally.",
        );
  ...
  private initializeMixpanel() {
    try {
      ...
      if (typeof window === "undefined" || typeof document === "undefined") {
        console.warn(
          "Mixpanel initialization skipped: not running in a browser environment",
        );
        return;
      }
      ...
      const initMixpanel = () => {
        ...
        if (!mixpanel || typeof mixpanel.init !== "function") {
          console.warn(
            "Mixpanel library loaded but global mixpanel object is unavailable",
          );
          return;
        }
      ...
      if (
        ...
        typeof (window as any).mixpanel.init === "function"
      ) {
        initMixpanel();
        return;
      }
      ...
      if (!document.querySelector("script[data-mixpanel-loader]")) {
        ...
        script.onerror = () => {
          console.warn(
            "Failed to load Mixpanel. Events will be stored locally.",
          );
          this.isMixpanelLoaded = false;
        };
  ...
  private processRateLimitQueue() {
    const now = Date.now();
    const itemsToRetry = this.rateLimitQueue.filter(
      (item) => now - item.timestamp > 60000, // Retry after 1 minute
    );
  ...
  private flushEventQueue() {
    if (
      (this.isGALoaded || this.isMixpanelLoaded) &&
  ...
  public async getUserPermissions(
    userId: string,
  ): Promise<{ canViewAnalytics: boolean; canViewDetailedMetrics: boolean }> {
    try {
  ...
  public trackDashboardInteraction(
    userId: string,
    action: string,
    element: string,
  ) {
